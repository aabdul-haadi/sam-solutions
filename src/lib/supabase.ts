...
// FIXED Consultation form submission
export const submitConsultationForm = async (data: {
  ...
  try {
    // CRITICAL FIX: Always provide values, NEVER null for required fields
    const sanitizedData = {
      name: sanitizeInput(data.name || ''),
      email: sanitizeInput((data.email || '').toLowerCase()),
      state: data.state ? sanitizeInput(data.state) : '', // Empty string, not null
      country: sanitizeInput(data.country || ''),
      phone: data.phone ? sanitizeInput(data.phone) : '', // Empty string, not null
      project_description: sanitizeInput(data.project_description || ''),
      preferred_day: data.preferred_day ? sanitizeInput(data.preferred_day) : '', // Empty string, not null
      preferred_time: data.preferred_time ? sanitizeInput(data.preferred_time) : '' // Empty string, not null
    };

    // Validate required fields
    if (!sanitizedData.name || !sanitizedData.email || !sanitizedData.country || !sanitizedData.project_description) {
      return { success: false, error: 'Please fill in all required fields' };
    }
